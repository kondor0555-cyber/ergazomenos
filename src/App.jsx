...
function LandingPage({ onLogin }) {
  return (
    <div style={{ fontFamily:"system-ui, sans-serif", color:COLORS.text }}>
      ...
      {/* NAV */}
      <nav style={{ background:COLORS.primary, padding:"0 24px", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ maxWidth:1100, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:64 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <span style={{ fontSize:28 }}>💼</span>
            <span style={{ color:"#fff", fontWeight:900, fontSize:22, letterSpacing:-0.5 }}>Ergan</span>
          </div>
          <button onClick={onLogin}
            style={{ background:COLORS.accent, color:"#fff", border:"none", borderRadius:10, padding:"10px 24px", fontWeight:700, fontSize:14, cursor:"pointer" }}>
            Είσοδος →
          </button>
        </div>
      </nav>
      ...
      {/* HERO */}
      <div style={{ background:`linear-gradient(135deg, ${COLORS.primary} 0%, #1a4a8c 60%, #2563a8 100%)`, padding:"80px 24px 100px", textAlign:"center" }}>
        <div style={{ maxWidth:700, margin:"0 auto" }}>
          <div style={{ display:"inline-block", background:"rgba(255,255,255,0.15)", borderRadius:20, padding:"6px 16px", fontSize:13, color:"#fff", marginBottom:20, fontWeight:600 }}>
            🇬🇷 Η νέα πλατφόρμα εργασίας για την Ελλάδα
          </div>
          <h1 style={{ color:"#fff", fontSize:"clamp(32px,5vw,52px)", fontWeight:900, margin:"0 0 20px", lineHeight:1.2, letterSpacing:-1 }}>
            Βρες δουλειά ή<br/>βρες προσωπικό<br/>
            <span style={{ color:COLORS.accentLight||"#f5c55a" }}>άμεσα & εύκολα</span>
          </h1>
          <p style={{ color:"rgba(255,255,255,0.8)", fontSize:18, lineHeight:1.7, marginBottom:36 }}>
            Μόνιμη, εποχική, ολίγων ημερών ή μερική απασχόληση.<br/>
            Απευθείας τηλεφωνική επικοινωνία με εργοδότες σε όλη την Ελλάδα.
          </p>
          <div style={{ display:"flex", gap:14, justifyContent:"center", flexWrap:"wrap" }}>
            <button onClick={onLogin}
              style={{ background:COLORS.accent, color:"#fff", border:"none", borderRadius:12, padding:"16px 36px", fontWeight:800, fontSize:16, cursor:"pointer", boxShadow:"0 4px 20px rgba(232,160,32,0.4)" }}>
              🔍 Ψάξε Αγγελίες
            </button>
            <button onClick={onLogin}
              style={{ background:"rgba(255,255,255,0.15)", color:"#fff", border:"2px solid rgba(255,255,255,0.4)", borderRadius:12, padding:"16px 36px", fontWeight:700, fontSize:16, cursor:"pointer" }}>
              📢 Δημοσίευσε Αγγελία
            </button>
          </div>
        </div>
      ...
      <div style={{ background:COLORS.card, borderBottom:`1px solid ${COLORS.border}` }}>
        <div style={{ maxWidth:900, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", padding:"32px 24px", gap:20 }}>
          {[["🏢","500+","Εργοδότες"],["💼","1.200+","Αγγελίες"],["👷","3.000+","Εγγεγραμμένοι"],["📍","50+","Πόλεις"]].map(([icon,num,label]) => (
            <div key={label} style={{ textAlign:"center" }}>
              <div style={{ fontSize:28 }}>{icon}</div>
              <div style={{ fontSize:28, fontWeight:900, color:COLORS.primary }}>{num}</div>
      ...
      {/* ΠΩΣ ΔΟΥΛΕΥΕΙ */}
      <div style={{ background:COLORS.bg, padding:"70px 24px" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          <h2 style={{ textAlign:"center", fontSize:32, fontWeight:900, marginBottom:8, color:COLORS.primary }}>Πώς λειτουργεί</h2>
          <p style={{ textAlign:"center", color:COLORS.muted, marginBottom:48, fontSize:16 }}>Τρία απλά βήματα για να βρεις αυτό που ψάχνεις</p>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))", gap:24 }}>
            {[
              ["1","📝","Εγγραφή","Δημιούργησε δωρεάν λογαριασμό ως εργαζόμενος, εργοδότης ή και τα δύο."],
              ["2","🔍","Αναζήτηση","Ψάξε με βάση πόλη, επάγγελμα και τύπο εργασίας."],
              ["3","📞","Επικοινωνία","Κάλεσε απευθείας τον εργοδότη με ένα κλικ. Χωρίς μεσάζοντες!"],
            ].map(([num,icon,title,desc]) => (
              <div key={num} style={{ background:COLORS.card, borderRadius:16, padding:28, border:`1.5px solid ${COLORS.border}`, textAlign:"center" }}>
                <div style={{ width:48, height:48, background:COLORS.primary, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px", color:"#fff", fontWeight:900, fontSize:18 }}>{num}</div>
                <div style={{ fontSize:32, marginBottom:12 }}>{icon}</div>
                <div style={{ fontWeight:800, fontSize:18, marginBottom:8, color:COLORS.primary }}>{title}</div>
                <div style={{ color:COLORS.muted, fontSize:14, lineHeight:1.7 }}>{desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ΓΙΑ ΕΡΓΑΖΟΜΕΝΟΥΣ / ΕΡΓΟΔΟΤΕΣ */}
      <div style={{ background:COLORS.card, padding:"70px 24px" }}>
        <div style={{ maxWidth:900, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:32 }}>
          <div style={{ background:`linear-gradient(135deg, #f0f9ff, #e0f2fe)`, borderRadius:20, padding:32, border:"1.5px solid #bae6fd" }}>
            <div style={{ fontSize:40, marginBottom:16 }}>👷</div>
            <h3 style={{ fontSize:22, fontWeight:800, color:COLORS.primary, marginBottom:16 }}>Για Εργαζόμενους</h3>
            {["Δωρεάν εγγραφή και χρήση","Χιλιάδες αγγελίες σε όλη την Ελλάδα","Άμεση επικοινωνία με εργοδότες","Φίλτρα για γρήγορη αναζήτηση","Μόνιμες, εποχικές & ευκαιριακές θέσεις"].map(f => (
            ...
            <button onClick={onLogin} style={{ marginTop:20, background:COLORS.primary, color:"#fff", border:"none", borderRadius:10, padding:"12px 24px", fontWeight:700, cursor:"pointer", width:"100%" }}>
              Ψάξε Δουλειά →
            </button>
          </div>
          <div style={{ background:`linear-gradient(135deg, #fff7ed, #ffedd5)`, borderRadius:20, padding:32, border:"1.5px solid #fed7aa" }}>
            <div style={{ fontSize:40, marginBottom:16 }}>🏢</div>
            <h3 style={{ fontSize:22, fontWeight:800, color:COLORS.primary, marginBottom:16 }}>Για Εργοδότες</h3>
            {["Εύκολη δημοσίευση αγγελιών","Άμεση επικοινωνία με υποψηφίους","Διαχείριση αγγελιών από το panel","Ευρεία κάλυψη σε όλη την Ελλάδα","Επείγουσες αγγελίες για άμεση πρόσληψη"].map(f => (
              <div key={f} style={{ display:"flex", gap:10, marginBottom:10, alignItems:"center" }}>
                <span style={{ color:"#ea580c", fontWeight:700 }}>✓</span>
                <span style={{ fontSize:14, color:COLORS.text }}>{f}</span>
              </div>
            ))}
            <button onClick={onLogin} style={{ marginTop:20, background:"#ea580c", color:"#fff", border:"none", borderRadius:10, padding:"12px 24px", fontWeight:700, cursor:"pointer", width:"100%" }}>
              Δημοσίευσε Αγγελία →
            </button>
          </div>
        </div>
      ...
      <div style={{ background:COLORS.bg, padding:"70px 24px" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          <h2 style={{ textAlign:"center", fontSize:32, fontWeight:900, marginBottom:8, color:COLORS.primary }}>Τύποι Εργασίας</h2>
          <p style={{ textAlign:"center", color:COLORS.muted, marginBottom:48, fontSize:16 }}>Βρες αυτό που ταιριάζει στις ανάγκες σου</p>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:16 }}>
            {[["🏢","Μόνιμη","Σταθερή απασχόληση","#1a7a45"],["🌊","Εποχική","Καλοκαίρι & χειμώνας","#1565c0"],["⚡","Ολίγων ημερών","Γρήγορη δουλειά","#b45309"],["🕐","Μερική","Ευέλικτο ωράριο","#7b1fa2"]].map(([icon,title,desc,color]) => (
              <div key={title} style={{ background:COLORS.card, borderRadius:14, padding:20, border:`2px solid ${color}20`, textAlign:"center" }}>
      ...
      <div style={{ background:COLORS.card, padding:"70px 24px" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          <h2 style={{ textAlign:"center", fontSize:32, fontWeight:900, marginBottom:8, color:COLORS.primary }}>Επικοινωνία</h2>
            ...
            <div>
              {[["📍","Διεύθυνση","Οδός Παράδειγμα 1, Αθήνα 10000"],["📞","Τηλέφωνο","210 000 0000"],["📧","Email","info@ergan.gr"],["🕐","Ώρες","Δευ-Παρ: 09:00-17:00"]].map(([icon,label,value]) => (
                <div key={label} style={{ display:"flex", gap:16, marginBottom:24, alignItems:"flex-start" }}>
                  ...
                  <div>
                    <div style={{ fontWeight:700, fontSize:14, color:COLORS.primary, marginBottom:2 }}>{label}</div>
                    <div style={{ fontSize:14, color:COLORS.muted }}>{value}</div>
                  </div>
                </div>
              ))}
            </div>
            <div style={{ borderRadius:16, overflow:"hidden", border:`1.5px solid ${COLORS.border}`, minHeight:280 }}>
              <iframe
                src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3145.0!2d23.7275!3d37.9838!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x14a1bd1f067043f1%3A0x2736354576668ddd!2sAthens!5e0!3m2!1sel!2sgr!4v1620000000000"
      ...
      {/* CTA */}
      <div style={{ background:`linear-gradient(135deg, ${COLORS.primary}, #1a4a8c)`, padding:"60px 24px", textAlign:"center" }}>
        <h2 style={{ color:"#fff", fontSize:32, fontWeight:900, marginBottom:12 }}>Έτοιμος να ξεκινήσεις;</h2>
        <p style={{ color:"rgba(255,255,255,0.8)", fontSize:16, marginBottom:32 }}>Εγγράψου δωρεάν και βρες αυτό που ψάχνεις σήμερα!</p>
        <button onClick={onLogin}
          style={{ background:COLORS.accent, color:"#fff", border:"none", borderRadius:12, padding:"16px 48px", fontWeight:800, fontSize:18, cursor:"pointer" }}>
          Ξεκίνα Δωρεάν →
        </button>
      </div>

      {/* FOOTER */}
      <div style={{ background:"#0f2236", padding:"32px 24px", textAlign:"center" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, justifyContent:"center", marginBottom:12 }}>
          <span style={{ fontSize:22 }}>💼</span>
          <span style={{ color:"#fff", fontWeight:900, fontSize:18 }}>Ergan</span>
        </div>
        <div style={{ color:"rgba(255,255,255,0.5)", fontSize:13 }}>
          © {new Date().getFullYear()} Ergan · Η πλατφόρμα εργασίας για την Ελλάδα
        </div>
      </div>
    </div>
  );
